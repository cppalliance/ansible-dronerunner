...
# is not sufficient on <version>. Install the Command Line Tools". Nothing here
# needs a source build today, bash pours a bottle, but the same ordering is
# what breaks bootstrap_mac_earlier_than_14.sh outright.
brew install bash
...
if [[ $(sw_vers -productVersion) =~ ^14 ]] ; then
    ...
    xcodeversions="14.2.0 14.3.0 14.3.1 15.0.0 15.0.1 15.1.0 15.2.0 15.3.0 15.4.0 16.0.0 16.1.0 16.2.0"
    gccversion="13"
    pythonversion="3.11"
    brew install python
    if [[ "$(uname -p)" == "arm" ]]; then
